...
> **定位**: CTF/攻防赛场景的前端 API 发现 + 未授权验证工具。集成 JS 提取、Webpack chunk、Vue/React 路由等多源 API 发现能力。AI 精判部分目前为人工操作（选目标、判结果），未集成到代码中。
...
Phase 2: JS 爬取 + API 提取
  ...
  - 路径拼接 (URL 目录 + API 路径)
Phase 3: 两阶段未授权测试
  - 3a: 快速筛选 (扁平线程池, TOP30 API × 2 种绕过)
  - 3b: 深度测试 (仅候选目标 × 全绕过方法)
Phase 4: 报告生成 (JSON + Markdown, 风险分级)
```
