...
# ===== CLI =====
parser = argparse.ArgumentParser(description='JS/API 未授权访问扫描器 v10')
parser.add_argument('--input', default='/tmp/v7_targets.json', help='目标JSON文件')
parser.add_argument('--outdir', default='/tmp/v10_scan_results', help='输出目录')
...
def main():
    print("="*60)
    print(f"v10: 文件下载专项+静态参数画像 | {'全量绕过' if args.full_bypass else '命中断路'} | 风险分级 | Markdown报告")
    ...
    print(f"\n[Phase 4] 报告生成")
    by_base = {v["base"]: v for v in load_checkpoint_results()}
    for v in vulnerable:
        by_base[v["base"]] = v
